...
bool CartesianImpedanceExampleController::init(hardware_interface::RobotHW* robot_hw,
  ...
  cmd_vel_sub_ = node_handle.subscribe<geometry_msgs::Twist>(
    "/cmd_vel", 10,
    &CartesianImpedanceExampleController::cmdVelCallback, this, ros::TransportHints().reliable().tcpNoDelay());
...
void CartesianImpedanceExampleController::starting(const ros::Time& /*time*/) {
  ...
  position_x = initial_transform.translation().x();
...
void CartesianImpedanceExampleController::update(const ros::Time& /*time*/,
  ...
  // commented this line as we are not changing position_d_target_ by keyboard commands
...
void CartesianImpedanceExampleController::equilibriumPoseCallback(
  ...
  // std::cout << "[gt positions] "
  //          << "Linear  -> x: " << msg->pose.position.x
...
void CartesianImpedanceExampleController::cmdVelCallback(
...
std::lock_guard<std::mutex> position_d_target_mutex_lock(
      ...
      double dt = (last_time_.isZero() ? 0.0 : (now - last_time_).toSec());
      last_time_ = now;

      if (dt > 0.0) {
        position_x +=  (msg->linear.x * dt) / 25;
        position_y +=  (msg->linear.z * dt) / 25;
      }

      position_d_.x() = position_x;
      position_d_.y() = position_y;
