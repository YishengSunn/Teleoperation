...
class CartesianImpedanceExampleController : public controller_interface::MultiInterfaceController<
 ...
 public:
  ...
  void setCommandQCallback(const std_msgs::Float64MultiArrayConstPtr &msg);
  void setCommandOCallback(const std_msgs::Float64MultiArrayConstPtr &msg);
 ...
 private:
  ...
  double translational_stiffness_{100};
  ...
  std::vector<double> command_;     // received position command from teleoeprator
  std::vector<double> command_o;     // received orientation command from teleoeprator
  std::vector<double> feedback_;    // slave feedback to send to teleoperator
  std::queue<std::vector<double>>commandQ;   // Position input queue
  std::queue<std::vector<double>>commandO;   // Orientation input queue
  std::mutex commandMutex;
  std::mutex commandMutex_O;
...
// To be sent to follower (feedback)
  ...
  double force_threshold = 3.5; // N

// Robot publisher and subscriber
  ros::Subscriber *F_Sub;   // position command subscriber from teleoeperator
  ros::Subscriber *O_Sub;   // orientation command subscriber from teleoeperator
